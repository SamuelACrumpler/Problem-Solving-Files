...
void linkedListType<Type>::divideAt(linkedListType<Type>& secondList, const Type & item)
{
	...
	while (current != NULL) {
		if (current->info == item) {//Sets Found to true when it encounters the value
			foundCheck = true;
		}
		if (foundCheck == true) {//While found is true
			newNode = new nodeType<Type>;
			newNode->info = current->info;
			newNode->link = NULL;
			//create the nodes for the second list.
			if (secondList.first == NULL)//Check if a first node exists
			{
				secondList.first = newNode;
				secondList.last = newNode;
			}
			else
			{
				secondList.last->link = newNode;
				secondList.last = newNode;
			}//end if
			 //delete the current node of the first list
			
			if (first->info == item) //Case 1
			{
				current = first;
				first = first->link;
				count--;
				...
				trailCurrent->link = current->link; //Takes the node ahead of current
				count--;
		...
		}// end foundCheck True IF
		else { //While found is false
