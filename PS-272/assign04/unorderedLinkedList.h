...
public:
	...
	void mergeLists(unorderedLinkedList<Type> &list1, unorderedLinkedList<Type> &list2);
	//This function creates a new list by merging the
	//elements of list 1 and list 2.
...
template<class Type>
void unorderedLinkedList<Type>::mergeLists(unorderedLinkedList<Type>& list1, unorderedLinkedList<Type>& list2)
{
	nodeType<Type> *firstCurrent, *secondCurrent;
	firstCurrent = list1.first;
	secondCurrent = list2.first;

	while (firstCurrent != NULL && secondCurrent != NULL) {
		if (firstCurrent->info < secondCurrent->info) {
			this->insertLast(firstCurrent->info);
			firstCurrent = firstCurrent->link;
		}//check if first is less than second
		else {
			this->insertLast(secondCurrent->info);
