...
(const linkedStackType<Type>& otherStack)
{
	nodeType<Type> *newNode, *current, *otherCurrent;
	bool blnCheck = true;

	current = this->stackTop;
	otherCurrent = otherStack.stackTop;

	while (current != NULL && otherCurrent != NULL) { //checks each value e
		if (current->info != otherCurrent->info)
			blnCheck = false;
		current = current->link;
		otherCurrent = otherCurrent->link;
	}
	if (current != NULL || otherCurrent != NULL) //Checks if the size of the stacks are even.
		blnCheck = false;
